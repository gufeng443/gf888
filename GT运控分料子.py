...
class FileSplitterApp:
    ...
    def merge_files(self):
        ...
        filter_dialog.title("过滤条件")
        filter_dialog.geometry("400x300")

        # 添加多行文本输入框
        label = tk.Label(filter_dialog, text="请输入要过滤的号码（每行一个）")
        label.pack(pady=10)

        text_area = tk.Text(filter_dialog, width=50, height=10)
        text_area.pack(padx=10, pady=5)

        # 处理确认按钮
        def on_confirm():
            filter_dialog.filter_text = text_area.get("1.0", tk.END).strip().splitlines()  # 存储到对话框对象
            filter_dialog.destroy()

        # 按钮框架
        btn_frame = tk.Frame(filter_dialog)
        btn_frame.pack(pady=10)

        confirm_btn = tk.Button(btn_frame, text="确定", command=on_confirm)
        confirm_btn.pack(side=tk.LEFT, padx=10)

        cancel_btn = tk.Button(btn_frame, text="取消", command=filter_dialog.destroy)
        cancel_btn.pack(side=tk.LEFT, padx=10)

        # 等待对话框关闭
        self.root.wait_window(filter_dialog)

        # 在等待对话框关闭后获取值：
        filter_text = getattr(filter_dialog, 'filter_text', None) or []
            ...
            if file_name.endswith(".txt"):
                ...
                try:
                    with open(file_path, "r", encoding="utf-8") as file:
                        ...
                        if filter_text:
                            # 修正过滤逻辑：转换为集合提高效率，并确保去除空白
                            filter_set = {item.strip() for item in filter_text}
                            file_data = [
                                line
                                for line in file_data
                                if line.strip() and line.strip() not in filter_set  # 添加空行检查
                            ]
